...
export default class ExtensionManager {
  ...
  get plugins() {
    // const _carbagePluginKeyCollector = [];
    return this.extensions
      .filter((extension) => "plugins" in extension)
      .reduce((allPlugins, { plugins }) => [...allPlugins, ...plugins], []);
    // .map(plugin => {
    //   if (_carbagePluginKeyCollector.includes(plugin.key)) {
    //     const newKey = plugin.key + Date.now();
    //     _carbagePluginKeyCollector.push(newKey);
    //     return Object.assign({}, plugin, {
    //       key: newKey,
    //     });
    //   }
    //   _carbagePluginKeyCollector.push(plugin.key);
    //   return plugin;
    // });
  }
